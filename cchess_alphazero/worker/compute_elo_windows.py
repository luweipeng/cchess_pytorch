...
class EvaluateWorker:
    ...
    def load_model(self, weight_path, digest, model_file=None):
        model = CChessModel(self.config)
        use_history = False
        if not model_file:
            use_history = False
            config_path = self.config.resource.model_best_path
        else:
            config_path = os.path.join(self.config.resource.model_dir, model_file)
