...
def replace_best_model(config):
    rc = config.resource
    shutil.copyfile(rc.next_generation_path, rc.model_best_path)
    # shutil.copyfile(rc.next_generation_weight_path, rc.model_best_weight_path)
    remove_ng_model(config)

def remove_ng_model(config):
    rc = config.resource
    os.remove(rc.next_generation_path)
    # os.remove(rc.next_generation_weight_path)

def load_model(config, model_path, name=None):
    model = CChessModel(config)
    if not load_model_weight(model, model_path, name):
        return None
    return model
