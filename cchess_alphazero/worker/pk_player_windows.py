...
class SelfPlayWorker:
    def __init__(self, config: Config):
        ...
        """
        self.config = config
        self.model1, self.use_history = self.load_model()
        # self.model1, self.use_history = self.load_model(model_file="model1.pth")
        self.model2, self.use_history = self.load_model(model_file="model2.pth")
        self.m = Manager()
        self.cur_pipes1 = self.m.list([self.model1.get_pipes() for _ in range(self.config.play.max_processes)])
        self.cur_pipes2 = self.m.list([self.model2.get_pipes() for _ in range(self.config.play.max_processes)])
    ...
    def start(self):
        ...
        with ProcessPoolExecutor(max_workers=self.config.play.max_processes) as executor:
            ...
            black_vic_num = 0
            # while True:
            for i in range(pk_round_num):
                game_idx += 1
                start_time = time()

                if len(futures) == 0:
                    for i in range(self.config.play.max_processes):
                        ff = executor.submit(self_play_buffer, self.config, self.cur_pipes1, self.cur_pipes2, self.use_history)
                ...
                thr_free.release()
            logger.info(f"对局完成：共对战{pk_round_num}次 "
                         f"红方胜利次数：{red_vic_num},黑方胜利次数：{black_vic_num} ")
    ...
    def load_model(self, model_file=None):
        ...
        if not model_file:
            ...
            model_path = os.path.join(self.config.resource.model_dir, model_file)
        try:
...
def self_play_buffer(config, cur1, cur2, use_history=False) -> (tuple, list):
    pipe1 = cur1.pop() # borrow
    pipe2 = cur2.pop()
    ...
    while not game_over:
        start_time = time()
        if turns % 2 == 0:
            action, policy = player1.action(state, turns, no_act, increase_temp=increase_temp)
        else:
            action, policy = player2.action(state, turns, no_act, increase_temp=increase_temp)
        end_time = time()
        if action is None:
