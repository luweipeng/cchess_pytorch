...
def load_best_model_weight(model):
    ...
    :return:
    """
    return model.load(model.config.resource.model_best_path)
...
def save_as_best_model(model):
    ...
    :return:
    """
    return model.save(model.config.resource.model_best_path)
...
def load_model_weight(model, model_path, name=None):
    if name is not None:
        logger.info(f"{name}: load model from {model_path}")
    return  model.load(model_path)

def save_as_next_generation_model(model):
    return model.save(model.config.resource.next_generation_path)
...
def load_sl_best_model_weight(model):
    ...
    :return:
    """
    return model.load(model.config.resource.sl_best_config_path)
...
def save_as_sl_best_model(model):
    ...
    :return:
    """
    return model.save(model.config.resource.sl_best_config_path)
